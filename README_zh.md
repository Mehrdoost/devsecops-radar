...
Pipeline Sentinel 是独立研究和开发的。如果您是个人开发者、学生或渗透测试者，在个人项目中使用免费的社区版，您可以帮忙让这个项目持续下去。

您的支持直接资助深夜开发、本地 AI 测试硬件以及持续的社区更新。

*（注意：在生产环境中部署此工具的企业实体必须获取上述商业许可证，而非依赖捐赠。）*
...
Pipeline Sentinel 基于 **Business Source License (BSL) 1.1** 发布。

我们相信在支持社区的同时，能够确保该项目的可持续发展：

* **🟢 社区与测试免费：** 您可以完全自由地以个人用途、教育目的、本地开发和测试环境使用、修改和运行本项目。
* **🔴 商业与生产环境使用：** 在组织的生产环境（例如，活跃的企业 CI/CD 管道、商业服务交付或企业安全运营）中使用 Pipeline Sentinel 需要获得 **商业许可证**。

⏳ **延时开源：** 为保障长期自由，此特定版本的代码将在 4 年后（2030 年 6 月 21 日）自动过渡到宽松的 **Apache 2.0 许可证**。
...
如果您代表一家公司，并希望在生产基础设施中部署 Pipeline Sentinel，我提供商业许可和专门的企业支持。
