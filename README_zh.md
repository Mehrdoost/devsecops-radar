...
想象一下有几个保安，每个人都在看守大楼的不同大门。他们都用不同的语言大喊他们的发现，而你必须到处跑才能了解发生了什么。

**Pipeline Sentinel** 将他们全部放在一个房间里，翻译他们的报告，并为你展示一个单一、清晰的全景屏幕。它连接到各种工具，如 **Trivy**（检查容器）、**Semgrep**（扫描代码）、**Poutine**（审计 GitLab 流水线）、**Zizmor**（保护 GitHub Actions）和 **Gitleaks**（查找硬编码凭证）。你无需挖掘多个 JSON 文件，即可获得一个**漂亮的深色模式仪表盘**，它会告诉你什么是关键问题，风险趋势如何，甚至攻击者如何将几个小问题链接成一次大攻击。
...
> **📌 图表占位符：** 请将你的网络流程图作为 `docs/network_flow.png` 添加到此处。  
...
| `--output` | 输出 JSON 文件（默认：findings.json） | `--output merged.json` |
...
FINDINGS_FILE=my.json devsecops-radar-web    # 使用自定义的扫描结果文件
...
*   自动检测 Trivy、Semgrep、Poutine、Zizmor 和纯列表格式
*   策略评估直接内置于引擎中
*   社区规则仓库：`devsecops-radar-rules`（可通过 `COMMUNITY_RULES_REPO` 配置）

### 🧠 LLM 驱动的分析
*   为不稳定的端点提供带指数退避的重试逻辑
*   涵盖真实世界供应链攻击链的 Few-shot 示例
*   感知 Token 的结果选择（最大项目数可通过 `ANALYZER_MAX_FINDINGS` 配置）
*   结构化 JSON 输出：`executive_summary`, `risk_score`, `attack_paths` (MITRE ATT&CK), `top_remediations`, `false_positives_likely`
*   支持 Ollama（本地、离线）和 LiteLLM（OpenAI、Anthropic 等）

### 🕸️ 多步攻击路径可视化
交互式 D3.js 物理图，将孤立的漏洞链接成真实的攻击场景。点击任何节点查看详细信息。接受拓扑文件，将结果映射到你的实际基础设施上，显示跨服务器和子网的横向移动。
...
```
*如果严重漏洞超过阈值，CLI 将以状态码 1 退出 — 非常适合用于阻断 CI/CD 流水线。*

### 🛠️ 带有人工循环的自动修复
AI 建议的修复可以自动应用 (`--fix`) 或逐一审查 (`--review`)。该工具会创建一个新的 git 分支并将其推送以供审核。还会生成一个 `fix.sh` 脚本，用于需要手动执行的命令。

### 📊 合规性与执行报告（带脱敏功能）
生成专业的 PDF 报告 (`--report report.pdf`)，包含：
*   执行摘要和风险评分
*   漏洞表格（前 50 项）
*   合规性映射（CIS、PCI‑DSS、ISO 27001）
*   自动脱敏密码、Token、JWT

### 📈 扫描历史记录和趋势（带分页）
支持服务器端分页的 SQLAlchemy 后端数据库 (`/api/findings?page=1&per_page=50`)。高效存储扫描历史记录，实现快速的趋势图表和历史对比。

### 🧪 SBOM & 依赖混淆检测
*   使用 `syft` 从你的项目生成 CycloneDX SBOM
*   检测 `package.json` 和 `requirements.txt` 中的依赖混淆风险 — 即可能被公共注册表冒充的内部包

### 🔍 RAG 驱动的安全搜索
使用自然语言询问有关扫描历史的问题：*“上一次发现 Log4j 漏洞是什么时候？”* 内置的 RAG 端点 (`/api/rag?q=...`) 会在存储的结果中搜索并返回匹配项。

### ⚔️ 攻击模拟（沙盒）
为任何漏洞生成简单的 PoC 脚本，并在一次性 Docker 容器中执行它，在不损害系统的情况下演示风险。

### 📉 动态风险评分
除 CVSS 外，每个漏洞都会根据资产暴露情况（基于拓扑）和漏洞利用可用性获得动态风险评分 — 帮助团队确定优先修复的内容。

### 🧙 交互式向导
`--wizard` 标志引导新用户完成安装依赖项、提取 AI 模型和运行首次扫描的过程 — 一气呵成。

### 🔒 隐私与离线优先
*   所有资源（CSS、JS）均已嵌入 — 无需 CDN 调用
*   LLM 分析通过 Ollama 在本地运行；数据不会离开你的网络
*   仪表盘提供可选的 API 密钥身份验证
...
| 阶段 | 功能 | 状态 |
| :--- | :--- | :--- |
| ✅ **阶段 1** | 多扫描器引擎 (Trivy, Semgrep, Poutine, Zizmor) | 已完成 |
| ✅ **阶段 1** | LLM 分析 (Ollama + LiteLLM) | 已完成 |
| ✅ **阶段 1** | 扫描历史记录，趋势图 | 已完成 |
| ✅ **阶段 1** | GitHub Action (composite) | 已完成 |
| ✅ **阶段 1** | Docker 镜像 (multi‑stage, non‑root) | 已完成 |
| ✅ **阶段 2** | 结合 MITRE ATT&CK 和拓扑的攻击路径可视化 | 已完成 |
| ✅ **阶段 2** | 策略即代码引擎 (`--policy`) | 已完成 |
| ✅ **阶段 2** | 自动修复引擎 (`--fix`) | 已完成 |
| ✅ **阶段 2** | 带自动脱敏的合规报告 (PDF) | 已完成 |
| ✅ **阶段 2** | 混合 RuleFusion 引擎（本地 + 社区规则） | 已完成 |
| ✅ **阶段 3** | Web 仪表盘 Blueprint 重构（模块化 Flask） | 已完成 |
| ✅ **阶段 3** | 真正基于 entry points 的扫描器插件系统 | 已完成 |
| ✅ **阶段 3** | 带分页功能的 SQLAlchemy ORM | 已完成 |
| ✅ **阶段 3** | SBOM & 依赖混淆检测 | 已完成 |
| ✅ **阶段 3** | 基于 RAG 的安全搜索 | 已完成 |
| ✅ **阶段 3** | 攻击模拟（沙盒） | 已完成 |
| ✅ **阶段 3** | 动态风险评分 | 已完成 |
| ✅ **阶段 3** | 交互式向导 (`--wizard`) | 已完成 |
| ✅ **阶段 3** | 人工审查模式 (`--review`) | 已完成 |
| ✅ **阶段 3** | Gitleaks 密钥扫描器 | 已完成 |
| ✅ **阶段 3** | 安全状态徽章端点 | 已完成 |
| 🔲 **阶段 4** | Jira / Slack 集成 | 计划中 |
| 🔲 **阶段 4** | SARIF & CycloneDX 支持 | 计划中 |
| 🔲 **阶段 4** | 规则市场（社区 YAML 规则） | 计划中 |
| 🔲 **阶段 4** | Pull Request 助手 (GitHub App) | 计划中 |

> 请参阅 [open issues](https://github.com/Mehrdoost/devsecops-radar/issues) 以获取提议功能的完整列表。
...
```
*此操作将合并结果，创建作业摘要，并输出严重/高危漏洞的计数。*

---

## 🤝 参与贡献

热烈欢迎提交 Pull requests 和 issues！
如果您想集成一个新的扫描器，请打开一个 issue 并附上其 JSON 输出样本。
对于永久性扫描器插件，请继承 `ScannerPlugin` 类并通过 entry points 注册。
