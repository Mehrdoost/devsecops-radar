...
14. [Улучшения безопасности в v0.4.2](#-улучшения-безопасности-в-v042)
...
## 🔐 Улучшения безопасности в v0.4.2

- **Предотвращение обхода пути (Path Traversal):** Все низкоуровневые операции с файлами манифестов, правил, SBOM и резервных копий проходят строгую валидацию путей для изоляции действий внутри разрешенной директории.
- **Очистка ввода для симуляции атак:** Механизмы создания скриптов эксплойтов экранируют любые управляющие данные пользователей, полностью исключая риски инъекции команд (Command Injection) за пределы песочницы.
- **Защищенная песочница Docker:** Скрипты симуляций принудительно изолируются внутри контейнеров с параметрами `--cap-drop=ALL`, файловой системой только для чтения `--read-only`, полным отключением сети `--network=none` и запускаются от имени непривилегированного пользователя `nobody`.
- **Проверка ключей доступа за константное время:** Модули авторизации и верификации API-ключей используют алгоритм `hmac.compare_digest`, что полностью защищает интерфейсы от атак по времени (Timing Attacks).
- **Безопасность соединений с базой данных:** По умолчанию для SQLite включен высокопроизводительный режим WAL, принудительно проверяются внешние ключи и настроен параметр `pool_pre_ping` для постоянного контроля здоровья долгоживущих коннектов.
- **Строгие лимиты на размерPayload:** Максимальный размер входящего тела запроса ограничен 1 МБ, а длина полей длинных форм автоматически усекается, что нивелирует риски DoS-атак и раздувания логов.
- **Безопасная синхронизация правил:** Операции git-клонирования жестко ограничены доверенным белым списком домена `[https://github.com](https://github.com)`, исключая передачу вредоносных параметров сборки.
- **Глубокое маскирование чувствительных данных:** Встроенный движок цензуры автоматически удаляет и скрывает пароли, приватные ключи и токены авторизации из логов сканирования и генерируемых PDF-отчетов.
- **Обязательные секреты окружения:** Ключ шифрования JWT и административный API-ключ проходят жесткую валидацию присутствия. При обнаружении пустых, дефолтных или слабых значений сервер выполняет защитное экстренное отключение (Fail-Fast).
