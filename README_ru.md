...
14. [Улучшения безопасности в v1.0.0](#-улучшения-безопасности-в-v100)
...
### 🔐 Опциональное шифрование базы данных

По умолчанию Pipeline Sentinel использует стандартную базу данных SQLite.  
Для изолированных или особо чувствительных сред можно включить
**прозрачное шифрование AES‑256** через `sqlcipher3`:

1. Установите дополнительную зависимость:  
   `pip install devsecops-radar[encryption]`
2. Задайте ключ шифрования в файле `.env`:  
   `DB_ENCRYPTION_KEY=ваш‑надёжный‑случайный‑ключ`
3. Запускайте приложение как обычно – база данных будет зашифрована
   на диске и расшифровываться «на лету».

Если расширение `encryption` не установлено или `DB_ENCRYPTION_KEY`
не задан, Pipeline Sentinel работает с обычной незашифрованной базой
(что совершенно нормально для локальной разработки и CI).
...
## 🛡️ Что нового в v1.0.0

Pipeline Sentinel v1.0.0 — это **полная переработка безопасности и архитектуры**
после шести месяцев интенсивного усиления. Каждый модуль прошёл аудит, был
переписан и укреплён для производственных DevSecOps‑процессов.

**🔒 Приоритет безопасности**
Полностью переработанная изоляция путей, защита от символических ссылок,
белые списки исполняемых файлов на основе каталогов, проверка GPG‑подписей
для правил сообщества, поддержка шифрованной базы данных и автоматическое
редактирование секретов во всех отчётах.

**🧠 Настоящий RAG и цепочки AI‑атак**
Семантический поиск по всем историческим находкам через ChromaDB + Ollama.
AI‑анализ теперь добавляет похожие прошлые находки в промпт, чтобы модель
могла распознавать многошаговые цепочки атак — именно то, что обещает README.

**🕸️ Иммерсивный 3D‑командный центр**
Новая киберпанк‑панель с 3D‑графами атак, горами трендов, городами топологии,
анимациями GSAP, процедурным звуком и **совершенно новой мини‑игрой простоя**
(Sentry Glider), которая запускается после двух минут бездействия.

**⚙️ Надёжные сканеры и CLI**
Все сканеры переписаны с обязательной проверкой путей и потоковым выводом.
`--fix` и `--review` безопасно применяют AI‑патчи с проверкой доказательств
и `patch --dry-run`. Полная поддержка Windows, macOS и Linux.

**🗃️ Единый слой данных**
Всё хранится в одной базе SQLite — больше никакого дрейфа JSON‑файлов.
Сканирования, AI‑сводки, топология и история всегда синхронизированы.
...
Pipeline Sentinel исследуется и разрабатывается независимо. Если вы индивидуальный разработчик, студент или пентестер, использующий бесплатный уровень для своих личных проектов, вы можете помочь поддержать жизнь этого проекта.

Ваша поддержка напрямую финансирует ночные разработки, локальное оборудование для тестирования ИИ и постоянные обновления для сообщества.

*(Примечание: корпоративные организации, внедряющие этот инструмент в производственных средах, должны приобрести Коммерческую лицензию выше, а не использовать пожертвования.)*
...
Pipeline Sentinel выпускается под лицензией **Business Source License (BSL) 1.1**.

Мы верим в поддержку сообщества, одновременно обеспечивая устойчивое развитие проекта:

* **🟢 Бесплатно для сообщества и тестирования:** Вы можете свободно использовать, изменять и запускать этот проект в личных целях, для обучения, локальной разработки и в тестовых средах.
* **🔴 Коммерческое и производственное использование:** Использование Pipeline Sentinel в производственных средах организаций (например, в активных корпоративных CI/CD-пайплайнах, при оказании коммерческих услуг или в корпоративных операциях безопасности) требует **Коммерческой лицензии**.

⏳ **Отсроченный открытый исходный код:** Чтобы гарантировать долгосрочную свободу, код этой конкретной версии автоматически перейдёт на разрешительную лицензию **Apache License 2.0** через 4 года (21 июня 2030 года).

Подробности см. в файле [LICENSE](LICENSE).

---

## 💼 Корпоративное и коммерческое лицензирование

Если вы представляете компанию и хотите развернуть Pipeline Sentinel в своей производственной инфраструктуре, я предлагаю коммерческое лицензирование и специализированную поддержку корпоративного уровня.

📫 **Свяжитесь для обсуждения лицензии:** [LICENSE]
