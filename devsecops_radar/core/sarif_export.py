...
def _get_safe_path(output_file: str, allowed_dir: str = ".") -> Path:
    ...
    if not target_path.is_relative_to(base_path):
        raise ValueError(f"Security Violation: Path traversal attempt detected in '{output_file}'")
    return target_path
...
def export_cyclonedx(findings: list[dict[str, Any]], output_file: str = "report.cdx.json") -> None:
    ...
    try:
        ...
        for f in findings:
            raw_target = str(f.get("target", "unknown"))
            # Generate a consistent bom-ref to link vulnerabilities to components
            comp_ref = f"pkg:file/{urllib.parse.quote(raw_target, safe='')}"
        ...
        cdx_data = {
            ...
            "version": 1,
            "metadata": {
                # ISO8601 UTC timestamp requirement
