...
14. [التحسينات الأمنية في الإصدار v0.4.3](#-التحسينات-الأمنية-في-الإصدار-v043)
...
## 🔐 التحسينات الأمنية في الإصدار v0.4.3

- **منع تجاوز المسار (Path Traversal):** تخضع جميع العمليات البرمجية الأساسية التي تتعامل مع ملفات القواعد، والمكونات، والـ SBOM، والنسخ الاحتياطي لتحقق مسبق يضمن بقاء العمليات داخل المجلد الأساسي المسموح به للتعامل الآمن.
- **تنقية المدخلات لمحاكاة الهجمات:** إدخال آليات هروب وحماية صارمة لبيانات التحكم داخل محركات إنشاء سكريبتات الاستغلال لتجنب مخاطر حقن الأوامر (Command Injection) خارج الحاوية.
- **حاوية Docker آمنة ومحمية:** تشغيل جميع سكريبتات المحاكاة الأمنية بشكل إجباري ومقيد عبر حاويات تمرر خيارات `--cap-drop=ALL`، ونظام ملفات للقراءة فقط `--read-only`، وعزل شبكي كامل `--network=none` وتعمل كمستخدم غير جذر `nobody`.
- **مقارنة مفاتيح المصادقة في وقت ثابت:** تعتمد بوابات تسجيل الدخول والتحقق من مفتاح الـ API على خوارزمية `hmac.compare_digest` لمنع هجمات التوقيت (Timing Attacks) على واجهات الحماية والوصول.
- **تأمين روابط قواعد البيانات:** تفعيل وضع WAL عالي الأداء لقواعد بيانات SQLite بشكل افتراضي، وفرض قيود المفاتيح الخارجية، وتكوين خيار `pool_pre_ping` لضمان فحص صحة واستمرارية الاتصال.
- **حدود حجم المدخلات الصارمة:** تقييد حجم حمولة البيانات للطلبات بحد أقصى 1 ميجابايت، واقتطاع أطوال حقول النماذج الطويلة لقطع الطريق تماماً على هجمات حجب الخدمة DoS وتضخم السجلات.
- **التحديث الآمن لقواعد المجتمع:** تقييد عمليات سحب وإحضار التحديثات من مستودعات Git لتكون ضمن النطاق الموثوق لقائمة البيضاء المحددة لـ `[https://github.com](https://github.com)` فقط لضمان عدم تمرير بارامترات خبيثة.
- **تشفير وحجب البيانات الحساسة والسرية:** تتضمن تقارير PDF وسجلات فحص الأنظمة محرك مراجعة صامت يقوم تلقائياً بحجب كلمات المرور وتوكنات الهوية والوصول والرموز الحساسة والمفاتيح الخاصة.
- **إجبارية المتغيرات البيئية السرية:** يتم التحقق الصارم من وجود مفتاح تشفير الـ JWT ومفتاح إدارة الـ API، وإذا تم رصد قيم فارغة أو ضعيفة أو افتراضية، يقوم النظام بعملية إغلاق سريع لحماية النظام (Fail-Fast).
