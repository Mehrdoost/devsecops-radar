...
14. [التحسينات الأمنية في الإصدار v0.4.4](#-التحسينات-الأمنية-في-الإصدار-v044)
...
```mermaid
graph LR
    subgraph Scanners [مدخلات الفحص المتعددة]
        T[فحص Trivy] 
        S[فحص Semgrep] 
        P[فحص Poutine] 
        Z[فحص Zizmor] 
    ...
    Scanners --->|التقارير الخام| CLI(🛡️ محرك devsecops-radar CLI)
    CLI --->|التوحيد وإزالة التكرار| Out[findings.json]
    Out ---> Web(📊 تطبيق لوحة Flask)
    Web ---> UI[🌐 مركز قيادة متصفح الويب الحديث]
...
## 🔐 تعزيز الأمان والجاهزية للإنتاج في الإصدار v0.4.4

هذا الإصدار هو إصلاح شامل للأمان والاستقرار. تم إصلاح جميع الثغرات المعروفة
في واجهة الويب و API ومنطق الفحص. يلبي المشروع الآن متطلبات الأمان على مستوى
المؤسسات، خاصةً في البيئات المعزولة (air‑gapped).

### 🔐 إصلاحات الأمان (حرجة وعالية الخطورة)
- XSS المخزّن – صفوف جداول لوحة التحكم وطرق العرض التفصيلية لم تعد تحقن
  بيانات الاكتشافات الخام في DOM؛ تم ترميز جميع القيم بشكل صحيح.
- اجتياز المسار – يتم التحقق من ملفات البيانات (مثل `findings.json` وملخصات
  الذكاء الاصطناعي والطوبولوجيا) لضمان بقائها داخل دليل العمل. تم حظر قراءة
  الملفات العشوائية عبر متغيرات البيئة.
- تجاوز المصادقة – نقاط النهاية `/api/attack-paths` و`/api/summary`
  و`/api/topology` و`/api/scan-result` تفرض الآن بشكل صحيح المصادقة بمفتاح
  API.
- كشف مفتاح API – لم تعد لوحة التحكم تعرض مفتاح API الحقيقي في مصدر الصفحة.
- حقن الأوامر في محاكاة الهجمات – توليد السكربت يسمح الآن فقط بمجموعة فرعية
  آمنة من الأحرف؛ يتم تجريد الأسطر المتعددة والأحرف الخاصة.
- تسرب الأسرار في التقارير – نتائج Gitleaks لم تعد تحتوي على السر الفعلي في
  الوصف؛ تقوم تقارير PDF/HTML بإخفاء أنماط الرموز الشائعة (GitHub وGitLab
  وAWS وJWT) عبر جميع الحقول.

### 🧱 البنية والأداء
- تحديد المعدل – تم تحديد معدل طلبات تسجيل الدخول وجميع نقاط النهاية المحمية
  بمفتاح API (في الذاكرة وآمنة للخيوط).
- إدارة الجلسات – لم تعد جلسة نطاق قاعدة البيانات تُغلق يدويًا، مما يمنع
  أخطاء "instance not bound" في الطلبات متعددة الخطوات.
- مخطط قاعدة البيانات – يتم تخزين `execution_time` الآن كـ `Float` (ثوانٍ)؛
  تمت إزالة العمود الزائد `findings_json`؛ وأضيف فهرس على `scans.timestamp`.
- محلل الذكاء الاصطناعي – تم تحديد معالجة الأجزاء المتزامنة بـ 5؛ تم تقييد
  نقطة نهاية Ollama بـ `localhost` لضمان الامتثال للعزل.
- محرك السياسات – حقل `on_violation` الجديد (`warn` / `fail`) يسمح بتطبيق
  مرن للسياسات.

### 🛡 تحسينات الماسحات الضوئية
- جميع الماسحات (`trivy` و`semgrep` و`zizmor` و`poutine` و`gitleaks`)
  تستدعي الآن `_validate_target_path` قبل تحليل أي ملف، مما يحظر اجتياز
  المسار على طبقة المهايئ.
- يدعم `Trivy` الآن ملخصات الصور (`repo@sha256:...`).
- يعالج `Semgrep` و`Gitleaks` بشكل صحيح أكواد الخروج غير الصفرية التي تشير
  إلى وجود نتائج.

### 🎨 المطور والعمليات
- تم نقل `semgrep` إلى التبعيات الاختيارية
  (`pip install devsecops-radar[extra]`).
- تم تمكين `Ruff` مع قواعد الأمان (S)؛ تم ضبط طول السطر على 120.
- يمكن تكوين مفتاح مشروع Jira ونوع المشكلة عبر متغيرات البيئة
  `JIRA_PROJECT_KEY` و`JIRA_ISSUE_TYPE`.
- يتم التحكم في مصادر CORS عبر متغير البيئة `CORS_ORIGINS` (لم يعد `*`).
- تم تنسيق جميع ملفات Python لتتوافق مع المعيار الجديد.

### ⬆️ تعليمات الترقية
1. اسحب أحدث صورة أو قم بتحديث الحزمة:
   `pip install --upgrade devsecops-radar`
2. راجع ملف `.env` – تأكد من أن `JWT_SECRET` و`PIPELINE_API_KEY` قوية
   (≥ 32 حرفًا).
3. إذا كنت تستخدم Jira، فقم بتعيين `JIRA_PROJECT_KEY` و`JIRA_ISSUE_TYPE`.
