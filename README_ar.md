...
14. [التحسينات الأمنية في الإصدار v1.0.0](#-التحسينات-الأمنية-في-الإصدار-v100)
...
## 🛡️ الجديد في الإصدار v1.0.0

يُعد Pipeline Sentinel v1.0.0 **إعادة هيكلة شاملة للأمان والبنية**
بعد ستة أشهر من التحصين المكثف. تم تدقيق كل وحدة، وإعادة كتابتها،
وتحصينها لسير عمل DevSecOps الإنتاجي.

**🔒 الأمان أولاً**
عزل مسار مُعاد تصميمه بالكامل، حماية من الروابط الرمزية، قائمة بيضاء
للتنفيذيين بناءً على الدليل، التحقق من توقيع GPG لقواعد المجتمع، دعم
قاعدة بيانات مشفرة، وإخفاء تلقائي للأسرار في كل تقرير.

**🧠 RAG حقيقي وسلاسل هجوم بالذكاء الاصطناعي**
بحث دلالي عبر جميع النتائج التاريخية عبر ChromaDB + Ollama.
يقوم تحليل الذكاء الاصطناعي الآن بحقن نتائج سابقة مشابهة في الموجه
ليتمكن النموذج من التعرف على سلاسل الهجوم متعددة الخطوات — تمامًا
كما يَعِد به ملف README.

**🕸️ مركز قيادة ثلاثي الأبعاد غامر**
لوحة تحكم جديدة بأسلوب السايبربانك مع رسوم بيانية ثلاثية الأبعاد للهجمات،
جبال الاتجاهات، مدن الطوبولوجيا، رسوم متحركة GSAP، صوت إجرائي،
و **لعبة صغيرة جديدة تمامًا للتسلية** (Sentry Glider) تبدأ بعد دقيقتين
من عدم النشاط.

**⚙️ ماسحات ضوئية و CLI صلبة كالصخر**
أُعيدت كتابة جميع الماسحات مع التحقق الإلزامي من المسار والإخراج المستمر.
يطبق `--fix` و`--review` تصحيحات الذكاء الاصطناعي بأمان مع التحقق من الأدلة
و`patch --dry-run`. يدعم بشكل كامل أنظمة Windows وmacOS وLinux.

**🗃️ طبقة بيانات متسقة**
كل شيء موجود في قاعدة بيانات SQLite واحدة — لا مزيد من انزياح ملفات JSON.
الفحوصات، ملخصات الذكاء الاصطناعي، الطوبولوجيا والسجل دائمًا متزامنة.
...
يتم البحث والتطوير في Pipeline Sentinel بشكل مستقل. إذا كنت مطورًا فرديًا أو طالبًا أو مختبر اختراق تستخدم الإصدار المجتمعي المجاني لمشاريعك الشخصية، فيمكنك المساعدة في استمرار هذا المشروع.

يدعم دعمك بشكل مباشر التطوير في ساعات الليل المتأخرة، وأجهزة اختبار الذكاء الاصطناعي المحلية، والتحديثات المستمرة للمجتمع.

*(ملاحظة: يجب على الكيانات المؤسسية التي تنشر هذه الأداة في بيئات الإنتاج الحصول على ترخيص تجاري أعلاه، بدلاً من استخدام التبرعات.)*
...
تم إصدار Pipeline Sentinel بموجب **Business Source License (BSL) 1.1**.

نحن نؤمن بدعم المجتمع مع ضمان التطوير المستدام لهذا المشروع:

* **🟢 مجاني للمجتمع والاختبار:** أنت حر تمامًا في استخدام هذا المشروع وتعديله وتشغيله للاستخدام الشخصي والأغراض التعليمية والتطوير المحلي وبيئات الاختبار.
* **🔴 الاستخدام التجاري والإنتاجي:** يتطلب استخدام Pipeline Sentinel في بيئات الإنتاج بالمؤسسات (على سبيل المثال، خطوط أنابيب CI/CD النشطة في الشركات، أو تقديم الخدمات التجارية، أو عمليات الأمان المؤسسية) **ترخيصًا تجاريًا**.

⏳ **مصدر مفتوح مؤجل:** لضمان الحرية على المدى الطويل، سينتقل كود هذا الإصدار المحدد تلقائيًا إلى الترخيص المتساهل **Apache License 2.0** بعد 4 سنوات (في 21 يونيو 2030).

يرجى الاطلاع على ملف [LICENSE](LICENSE) للحصول على التفاصيل الكاملة.

---

## 💼 الترخيص المؤسسي والتجاري

إذا كنت تمثل شركة وترغب في نشر Pipeline Sentinel داخل البنية التحتية للإنتاج لديك، فأنا أقدم ترخيصًا تجاريًا ودعمًا مؤسسيًا مخصصًا.

📫 **تواصل لمناقشة الترخيص:** [LICENSE]
